...
# Download dos arquivos .cram, .crai e BED (somente se ainda não existem)
[[ -f "$CRAM" ]] || wget -O "$CRAM" "$CRAM_URL"
[[ -f "$CRAI" ]] || wget -O "$CRAI" "$CRAI_URL"
[[ -f "$BED"  ]] || wget -O "$BED"  "$BED_URL"

# Download da referência genômica (mesma usada para gerar o arquivo .cram - se ainda não estiver presente)
[[ -f "$REF_FASTA" ]] || wget -O "$REF_FASTA" \
...
[[ -f "${REF_FASTA}.fai" ]] || wget -O "${REF_FASTA}.fai" \
